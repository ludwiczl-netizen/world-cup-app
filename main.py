...
from fastapi.responses import HTML["gol2"] is not None:from fastapi.responses import HTMLResponse, RedirectResponse
...
def home():
    ...
    for i, r in enumerate(ranking, 1):
        ...
        rows += f"""
        <tr onclick="location.href='/gracz/{safe}'" class="row/td>
            <td>{r['name']}</td>
...
def player(name: str):
    ...
    for _, r in df.iterrows():
        ...
        if actual:
            ...
            html += f"""
            <div style="background:white;padding:12px;margin:8px;border-radius:12px;
